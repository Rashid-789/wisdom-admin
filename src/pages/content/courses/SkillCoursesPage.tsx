...
export default function SkillCoursesPage() {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      const res = await listCoursesByBucket("skillCourse", { page, pageSize, search, status: "all" });
  ...
  React.useEffect(() => {
    void load().catch((error) => {
      toast.error(getApiErrorMessage(error, "Failed to load skill courses"));
    });
  }, [load]);
        ...
        <CardContent className="space-y-4 p-4 sm:p-6">
          ...
          <DataTable
            ...
            columns={[
              ...
              { key: "lecturerName", header: "Lecturer", cell: (row) => row.lecturerName ?? "-" },
                  ...
                  <div className="flex justify-end">
                    <RowActionsMenu
                      ...
                      onDelete={async () => {
                        ...
                        try {
                          await deleteCourseCascade("skillCourse", row.id);
                          await load();
            ...
            emptyTitle="No skill courses found"
            emptyDescription="Create a skill course to build chapters and topics."
          />
